"""
This module contains the novelty agent class. 
"""
import agent, kUtil
import numpy as np
...
class novelty(agent.agent):
	...
	def __init__(self, worldRef, simIndex, noisyPos, sigma, agentType, noisyBallPos,maxPlayerSpeed, maxBallSpeed, posession = False):
		...
		fileExists = os.path.isfile('novelty_Population/genome.txt')
		if fileExists:
			...
			g = MNEAT.Genome('novelty_Population/genome.txt')
	...
	def _decisionFunction(self):
		...
		holdListLength = len(self.holdList)
		if holdListLength == 0:
			self.holdList.append(self.bevSubstrate[holdDecision])
		elif (self.holdList[holdListLength - 1][0] != self.bevSubstrate[holdDecision][0]) or (self.holdList[holdListLength - 1][1] != self.bevSubstrate[holdDecision][1]):
			self.holdList.append(self.bevSubstrate[holdDecision])
		passList = [0,0]
