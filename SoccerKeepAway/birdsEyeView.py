...
class birdsEyeView():
    ...
    """
    def __init__(self, display_width, display_height, block_size = 23, ball_size = 12):
    ...
    def getBirdsEyeViewAsList(self, keeperArray, takerArray):
        grid = self.getBirdsEyeView(keeperArray, takerArray)
        returnlist = []
        for l in grid:
            returnlist = returnlist + l
        return returnlist
    
    def getSubstrate(self, keeperArray, takerArray):
        grid = self.getBirdsEyeView(keeperArray, takerArray)
    ...
    def getBallHolderTile(self, keeperArray):
        """
        This function will take in the array of keepers, and return the index of keeper 0 
        in the substrate. The substrate is generated by the getSubstrateMethod of this
        class
        :param keeperArray: the array of keepers. should be taken in from simulator
        
        :type keeperArray: this is a list of agents, where each agent is an instance of agent.py
        
        :returns: index of the substrate that corresponds to keeper 0, the keeper with the ball.
        :rtype: integer
        """
        numCols =  int(math.ceil(self.display_width/self.__block_size))
        sortedKeepers = sorted(keeperArray)
        tile =  self.__convertToTile(sortedKeepers[0].getNoisyMidPoint())
        return int(round(tile[0] * numCols + tile[1]))
        
    
    def getBirdsEyeView(self, keeperArray, takerArray):
        """
        ...
        for i in range(int(math.ceil(self.display_height/self.__block_size))):
            returnGrid.append([]) 
            for j in range(int(math.ceil(self.display_width/self.__block_size)) ):
