...
class SarsaAgent(agent):
    ...
    def computeRandomValueFromQValues(self, state):
        """
          Returns max_action Q(state,action)
          where the max is over legal actions.  Note that if
          there are no legal actions, which is the case at the
    ...
    def update(self, state, action, nextState, reward):
        """
          ...
          """
        self.q_values[(state,action)]=((1-self.alpha)*(self.q_values[(state,action)]))+((self.alpha)*(reward+(self.discount*self.computeRandomValueFromQValues(nextState))))
        return self.q_values[(state,action)]
    ...
    def getValue(self, state):
        return self.computeRandomValueFromQValues(state)
